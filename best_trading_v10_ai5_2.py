...
class Config:
    ...
    SMART_OHLCV_FETCH: bool = True           # fetch only on new bar
    BALANCE_REFRESH_S: int = 300             # fetch_balance interval (5 min)
    UNIVERSE_REFRESH_H: int = 24             # fetch_tickers interval (24h)
...
def execute_post_only(exchange, symbol: str, side: str, qty: float,
    ...
    try:
        while time.time() - t0 < wait:
            ...
            # 2. Compute target (1 bps beyond touch, matching backtest penetration)
            if side == 'buy':
...
# ════════════════════════════════════════════════════════════════
# § 18.91  Smart OHLCV Refresh Detection
# ════════════════════════════════════════════════════════════════
...
def _needs_ohlcv_refresh(cached_df, tf_sec: int) -> bool:
    ...
    Detection: compare the open-time of the last cached bar with
    the open-time of the current wall-clock bar.

    If they differ, a new bar opened → refresh required.
    Else, no new data → skip fetch.
    """
    ...
    try:
        ...
        current_open_ts = (now_ts // tf_sec) * tf_sec
...
def run_live(cfg, exchange):
    ...
    while True:
        try:
            ...
            # ══ [BANDWIDTH] Refresh universe every UNIVERSE_REFRESH_H hours ══
            _univ_h = getattr(CFG, 'UNIVERSE_REFRESH_H', 24)
            if time.time() - last_scan_time > _univ_h * 3600:
                log.info(f"🔄 تحديث قائمة الأصول (كل {_univ_h}h)...")
                top_syms = scan_top_assets(exchange)
                cached_data = fetch_all(top_syms, exchange, cfg.timeframe, days=7, workers=5)
                last_scan_time = time.time()
                corr_cache.clear()

            # ══ [BANDWIDTH] Throttle fetch_balance (every BALANCE_REFRESH_S) ══
            if not hasattr(run_live, '_last_balance_ts'):
                run_live._last_balance_ts = 0.0
            if not hasattr(run_live, '_cap_live'):
                run_live._cap_live = cfg.INITIAL_CAPITAL

            if (time.time() - run_live._last_balance_ts) >= CFG.BALANCE_REFRESH_S:
                try:
                    bal = exchange.fetch_balance()
                    run_live._cap_live = float(bal['USDT']['free'])
                except Exception:
                    pass
                run_live._last_balance_ts = time.time()
            cap_live = run_live._cap_live
            ...
            assets = {}
            # ══ [BANDWIDTH] Smart OHLCV fetch: only when new bar opens ══
            _tf_sec = CFG.TF_SECONDS if CFG.TF_SECONDS > 0 else 3600
            _smart_fetch = getattr(CFG, 'SMART_OHLCV_FETCH', True)
            _skipped = 0
            _fetched = 0
            ...
            for sym in top_syms:
                ...
                try:
                    # Smart skip: if no new bar opened since last fetch, reuse cache
                    if _smart_fetch and not _needs_ohlcv_refresh(cached_data[sym], _tf_sec):
                        _skipped += 1
                    else:
            ...
            # ══ [BANDWIDTH] Log stats occasionally ══
            if _fetched > 0 or _skipped > 0:
                if not hasattr(run_live, '_bw_last_log'):
                    run_live._bw_last_log = 0.0
                if time.time() - run_live._bw_last_log > 600:  # every 10 min
                    log.info(f"[Bandwidth] ohlcv: fetched={_fetched}, "
                             f"skipped={_skipped} (saved {_skipped*500/1024:.1f} KB this cycle)")
                    run_live._bw_last_log = time.time()
...
def main():
    ...
    p.add_argument("--no-smart-ohlcv", action="store_true",
                   help="Disable smart OHLCV fetch (fetch every cycle)")
    p.add_argument("--balance-refresh-s", type=int, default=None,
                   help="Seconds between fetch_balance calls (default 300)")
    p.add_argument("--universe-refresh-h", type=int, default=None,
                   help="Hours between fetch_tickers calls (default 24)")
    args = p.parse_args()
    ...
    if args.no_smart_ohlcv:
        CFG.SMART_OHLCV_FETCH = False
    if args.balance_refresh_s is not None:
        CFG.BALANCE_REFRESH_S = int(args.balance_refresh_s)
    if args.universe_refresh_h is not None:
        CFG.UNIVERSE_REFRESH_H = int(args.universe_refresh_h)

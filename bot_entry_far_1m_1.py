...
CACHE_DIR = "market_data_cache_1m"
...
class Config:
    ...
    ASSET_CACHE_DIR: str = "asset_cache_1m"
    ...
    # ══ [ADV-AWARE NOTIONAL CAP] ══
    MAX_ADV_IMPACT_FRAC: float = 0.001     # max 0.1% of daily ADV
    # ══ [SIGMA-SCALED TRAIL] ══
    TRAIL_SIGMA_SCALED: bool = True
    TRAIL_ACT_KAPPA: float = 0.40       # activation = 0.40 × σ_bar
    TRAIL_DIST_KAPPA: float = 0.30      # distance   = 0.30 × σ_bar
    TRAIL_SIGMA_FLOOR_PCT: float = 0.0010  # abs floor (0.10% of price)
    TRAIL_SIGMA_CEIL_PCT: float = 0.0080   # abs ceiling (0.80%)
    # ══ [TF-AWARE GATE] ══
    ENTRY_MAX_AGE_BARS_1H: int = 2         # reference for 1h
    ENTRY_MAX_AGE_SCALE_WITH_TF: bool = True
    # ══ [COMPOUNDING GUARD] ══
    COMPOUND_GUARD_ENABLED: bool = True
    COMPOUND_GUARD_CAP_MULT: float = 100.0    # cap final at 100× initial
    ...
    # ══ [CLOSE-ENTRY MODE] ══
    FRICTION_DRAG_MULT: float = 0.25      # was 0.50 in build_signals
    ENTRY_LADDER_TIGHT: bool = True        # tighten ladder offsets
    ENTRY_LADDER_OFFSETS_TIGHT: Tuple = (0.0, 0.25, 0.50)
...
def build_signals(assets, mode="backtest"):
    ...
    for sym, ad in assets.items():
        ...
        for fi in range(ad.train_end+1, n):
            ...
            _drag_mult = float(getattr(CFG, 'FRICTION_DRAG_MULT', 0.50))
            friction_drag = _sigma * p * 0.1
...
def _trail_dist_for_bar(ad, current_fi: int) -> Tuple[float, float]:
    """
    σ-scaled (activation, distance) for trailing on the current bar.
    Works across timeframes without hardcoded percentages.
    Returns (activate_frac, distance_frac) as fractions of price.
    """
    sigma = 0.01
    try:
        if ad.E_therm is not None and 0 <= current_fi < len(ad.E_therm):
            _s = float(ad.E_therm[current_fi])
            if np.isfinite(_s) and _s > 1e-6:
                sigma = _s
    except Exception:
        pass

    if not getattr(CFG, 'TRAIL_SIGMA_SCALED', True):
        return float(CFG.TRAIL_ACTIVATE_MFE), float(CFG.TRAIL_DISTANCE)

    floor = float(getattr(CFG, 'TRAIL_SIGMA_FLOOR_PCT', 0.0010))
    ceil = float(getattr(CFG, 'TRAIL_SIGMA_CEIL_PCT', 0.0080))

    act = float(np.clip(CFG.TRAIL_ACT_KAPPA * sigma, floor * 0.5, ceil))
    dst = float(np.clip(CFG.TRAIL_DIST_KAPPA * sigma, floor * 0.3, ceil))
    return act, dst
...
def _advance(pos, ad, to_ci):
    ...
    for cidx in range(start, end+1):
        ...
        # ── Trailing Stop (σ-scaled, per-bar) ──
        _tf_fi = max(0, cidx - ad.feat_start)
        _ta_dyn, _td_dyn = _trail_dist_for_bar(ad, _tf_fi)
        _ta = _ta_dyn if _ta_dyn > 0 else CFG.TRAIL_ACTIVATE_MFE
...
def _adv_aware_max_notional(ad, idx: int) -> float:
    """
    min(MAX_ABS_NOTIONAL, ADV × MAX_ADV_IMPACT_FRAC).
    Keeps compounding blowup under control on illiquid assets.
    """
    try:
        adv_daily = float(ad.adv_usd[min(max(idx, 0), len(ad.adv_usd)-1)])
        frac = float(getattr(CFG, 'MAX_ADV_IMPACT_FRAC', 0.001))
        cap_adv = max(1.0, adv_daily * 24.0 * frac)
    except Exception:
        cap_adv = float(CFG.MAX_ABS_NOTIONAL)
    cap_abs = float(getattr(CFG, 'MAX_ABS_NOTIONAL', 100_000.0))
    return float(min(cap_abs, cap_adv))
...
def simulate_portfolio(signals, assets, corr_matrix, mode="backtest"):
    ...
    _compound_cap = (CFG.INITIAL_CAPITAL *
                     float(getattr(CFG, 'COMPOUND_GUARD_CAP_MULT', 100.0)))
    ...
    for sig_i, sig in enumerate(signals):
        ...
        # ══ [COMPOUND GUARD] stop trading if unrealistic growth ══
        if (getattr(CFG, 'COMPOUND_GUARD_ENABLED', True)
                and capital >= _compound_cap):
            log.warning(f"[CompoundGuard] capital {capital:.2f} >= cap "
                        f"{_compound_cap:.2f} — stopping backtest at "
                        f"signal {sig_i}/{len(signals)}")
            break
        ...
        # ══ [ADV-AWARE CAP] prevents impact blowup ══
        _adv_cap = _adv_aware_max_notional(ad, opt_ci)
        qty = min(qty, _adv_cap / opt_px)
...
_ASSET_CACHE_DIR = "asset_cache_1m"
...
def _effective_max_age_bars() -> int:
    """
    Age limit in bars, scaled so the wall-clock window stays ~2h.
    On 1m: 120 bars.  On 1h: 2 bars.  On 15m: 8 bars.
    """
    if not getattr(CFG, 'ENTRY_MAX_AGE_SCALE_WITH_TF', True):
        return int(getattr(CFG, 'ENTRY_MAX_AGE_BARS', 2))
    tf_sec = CFG.TF_SECONDS if CFG.TF_SECONDS > 0 else 3600
    # target ≈ 2h of bars
    target_sec = 2 * 3600
    n_bars = max(1, int(round(target_sec / tf_sec)))
    # clamp to reasonable range
    return int(np.clip(n_bars, 1, 500))
...
def validate_entry_signal(sig, ad, current_ci: int) -> Tuple[bool, str]:
    ...
    max_age = _effective_max_age_bars()
    if age > max_age:
...
def build_entry_ladder(sig, ad, current_ci: int) -> List[Dict]:
    ...
    if getattr(CFG, 'ENTRY_LADDER_TIGHT', False):
        offsets = getattr(CFG, 'ENTRY_LADDER_OFFSETS_TIGHT', (0.0, 0.25, 0.50))
    else:
    ...
    while True:
        try:
            ...
            for sym in list(open_pos_live.keys()):
                ...
                if not ex:
                    entry_px = pos['entry']
                    _cur_fi_t = max(0, len(ad.closes) - 2 - ad.feat_start)
                    _ta_dyn_t, _td_dyn_t = _trail_dist_for_bar(ad, _cur_fi_t)
                    _ta = float(_ta_dyn_t if _ta_dyn_t > 0
                                 else pos.get('trail_activate_frac', CFG.TRAIL_ACTIVATE_MFE))
            ...
            if len(open_pos_live) < effective_max:
                ...
                for sig in reversed(sigs):
                    ...
                    # ══ [ADV-AWARE CAP] in live ══
                    _adv_cap = _adv_aware_max_notional(assets[sym], _entry_ci)
                    qty = min(qty, _adv_cap / lmt)

...
def _load_cached(symbol, exchange, timeframe, days):
    ...
    if os.path.exists(fp):
        ...
        since   = exchange.parse8601(
            (last_ts+timedelta(hours=1)).isoformat()+"Z")
        rows = []
        try:
            while True:
...
def _advance(pos, ad, to_ci):
    ...
    for cidx in range(start, end+1):
        ...
        if cidx - pos.entry_ci > CFG.MAX_HOLD_BARS:
            pos.trail_sl = trail_sl; pos.current_ci = cidx
...
def simulate_portfolio(signals, assets, corr_matrix, mode="backtest"):
    ...
    fill_map = precompute_entry_fills(
        assets, signals,
        max_wait_bars=CFG.FILL_ENTRY_MAX_WAIT_BARS,
    ...
    def _close(pos, ad, exit_px, exit_rsn, exit_ci):
        ...
        funding_cost = (pos.pos_size * pos.entry_px * CFG.FUNDING_RATE_COST * funding_payments) if sig.action == "BUY" else 0.0
...
def _load_ml_filter() -> bool:
    ...
    try:
        ...
        n_expected = 52
        if len(_ML_FEATURE_NAMES) != n_expected:
            log.error(f"[ML] Feature count mismatch: model has "
                      f"{len(_ML_FEATURE_NAMES)}, bot expects {n_expected}")
            log.error(f"[ML] Retrain with ml_filter_trainer_v5.py")
            return False
...
def _extract_ml_features(sig, ad) -> np.ndarray:
    """52-feature extractor. Must mirror ml_filter_trainer_v5.extract_ml_features."""
    ci = sig.close_idx
    fi = sig.feat_idx
    ...
    try:
        ...
        if opens_arr is None:
            ...
            opens_arr[1:] = closes[:-1]

        p_now = float(closes[ci])
        o_now = float(opens_arr[ci])
        h_now = float(highs[ci])
        l_now = float(lows[ci])
        v_now = float(vols[ci])
        ...
        if ci >= 20:
            ...
            x = np.arange(len(v20), dtype=np.float64)
            xm = x.mean(); ym = float(np.mean(v20))
            num = float(np.sum((x - xm) * (v20 - ym)))
        ...
        if ci >= 20:
            ...
            dist_from_20_high = dist_from_20_low = 0.0
...
def filter_signals_ml(signals, assets):
    ...
    for sig in signals:
        ...
        if ad is None:
            ...
            continue
        try:
            feat = _extract_ml_features(sig, ad).reshape(1, -1)
...
def signal_still_valid(ad, sig, current_ci: int, max_age_bars: int = 3):
    ...
    # 2. Setup-invalidation: has market drifted so far the thesis is broken?
    #    For BUY:  entry is BELOW market. If market rallies >2% → dip unreachable.
    #                                If market collapses >3% → momentum broken.
    if sig.action == "BUY":
        if move_frac > 0.02:
            return False, f"rallied {move_frac*100:+.2f}% — dip unreachable"
        if move_frac < -0.03:
            return False, f"collapsed {move_frac*100:+.2f}% — momentum broken"
    else:  # SELL
        if move_frac < -0.02:
            return False, f"crashed {move_frac*100:+.2f}% — bounce unreachable"
        if move_frac > 0.03:
            return False, f"rallied {move_frac*100:+.2f}% — momentum broken"
...
def run_live(cfg, exchange):
    ...
    while True:
        try:
            ...
            assets = {}
            # التحديث التراكمي المستمر (دمج الشمعة غير المكتملة وحساب طوبولوجيا الطاقة الحية)
            for sym in top_syms:
            ...
            for sym in list(open_pos_live.keys()):
                ...
                if not ex:
                    ...
                    if entry_ts > 0:
                        bars_held = (time.time() - entry_ts) / (
                            60.0 if cfg.timeframe == '1m' else 3600.0
                        )
                ...
                if not ex:
                    entry_px = pos['entry']
                    if pos['action'] == "BUY":
                        pnl_frac = (price - entry_px) / entry_px
                        if pnl_frac > 0.01:
                            new_sl = entry_px * 1.002
                            if new_sl > pos['sl']:
                                pos['sl'] = new_sl
                    else:
                        pnl_frac = (entry_px - price) / entry_px
                        if pnl_frac > 0.01:
                            new_sl = entry_px * 0.998
                            if new_sl < pos['sl']:
                                pos['sl'] = new_sl
                ...
                try:
                    ...
                    if is_emergency:
                        ...
                        # Normal exit → Post-Only with market fallback
